...
for(i in 1:R){
  sampled_rows_sim1 <- sample(nrow(cces2), size = 1000) # we'll take a new sample of rows
  sampled_data_sim1 <- cces2[sampled_rows_sim1, c('ed_spend_num', 'total_spending')] # and take a new sample from the cces2
  model_samp_sim1 <- lm(ed_spend_num ~ total_spending, data = sampled_data_sim1) # fit the model again
  slope_ests_sim1[i] <- coef(model_samp_sim1)['total_spending'] # store the estimated slope as the ith slope estimate
  sample_sizes_sim1[i] <- model_samp_sim1$df.residual
  stderr_sim1[i] <- coef(summary(model_samp_sim1))['total_spending', 'Std. Error']
  if(i%%100 == 0) print(i) # print i every 100 samples; useful to see how fast the code is running
}
...
for(i in 1:R){
  sampled_states_sim2 <- sample(unique(cces2$state), size = 3)
  sampled_data_sim2 <- cces2[cces2$state %in% sampled_states_sim2, c('ed_spend_num', 'total_spending')]
  model_samp_sim2 <- lm(ed_spend_num ~ total_spending, data = sampled_data_sim2)
  slope_ests_sim2[i] <- coef(model_samp_sim2)['total_spending']
  sample_sizes_sim2[i] <- model_samp_sim2$df.residual
  stderr_sim2[i] <- coef(summary(model_samp_sim2))['total_spending', 'Std. Error']
  if(i%%100 == 0) print(i)
}

cat("true slope: ", coef(model1)['total_spending'], '\nmean from sim 1: ', mean(slope_ests_sim1), "\nmean from sim 2: ", mean(slope_ests_sim2))
cat("sample size in sim 1: ", mean(sample_sizes_sim1), "\nsample size in sim 2: ", mean(sample_sizes_sim2))
...
cces_zc <- right_join(census_dat_zc, cces2)
model5 <- lm(ed_spend_num ~ median_income + faminc_num + employ + gender + educ + race + child18, data = cces_zc)


#put this in later!! mod2.5 <- lm(prestg10 ~ age + I(age^2) + sex, data = gss)
# census$lookupzip <- str_remove(census$name, "zcta5")
...
#### Challenge Question ####
## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ## ##
>>>>>>> b32abe44b8ebbb41b623867fd995040e0802949d
>>>>>>> 7a6ac5e1c7961d393cff2ee03bf0c68e0062be66
